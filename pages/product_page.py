...
class ProductPage(BasePage):
    ...
    def correct_message_about_adding_to_cart(self): #проверяем что текст сообщения в плашке корректный
        ...
        assert title in message_about_adding_to_cart, f"В сообщении '{message_about_adding_to_cart}' нет названия товара '{title}'"
    ...
    def correct_message_about_the_cart_cost(self): #проверяем что стоимость корзины в плашке указана корректно
        ...
        assert price in message_about_adding_the_cart_cost, f"В сообщении '{message_about_adding_the_cart_cost}' должна быть указана цена '{price}'"
